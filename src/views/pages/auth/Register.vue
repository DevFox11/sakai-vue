...
const handleRegister = async () => {
    ...
    if (!email.value || !emailRegex.test(email.value)) {
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Por favor ingresa un email válido (ej: usuario@dominio.com)',
            life: 3000
        });
        return;
    }
    ...
    try {
        // Llamar al authStore para registrar
        ...
        toast.add({
            ...
            detail: `¡Bienvenido ${authStore.userName}! Tu cuenta ha sido creada.`,
            life: 4000
        });

        // Redireccionar al dashboard (auto-login después del registro)
        router.push('/');
    } catch (error) {
        // Mostrar error
        const errorMessage = authStore.error || 'Error al crear la cuenta. Por favor intenta de nuevo.';
        toast.add({
            severity: 'error',
                ...
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-16 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        <svg viewBox="0 0 54 40" fill="none" xmlns="http://www.w3.org/2000/svg" class="mb-6 w-16 shrink-0 mx-auto">
                            ...
                            />
                            <mask id="mask0_register" style="mask-type: alpha" maskUnits="userSpaceOnUse" x="0" y="8" width="54" height="11">
                        ...
                        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Crea tu cuenta en Dayvo</div>
                        <span class="text-muted-color font-medium">Completa el formulario para registrarte</span>
                    </div>
                    ...
                    <div>
                        ...
                        <label for="email-reg" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Email</label>
                        <InputText 
                            id="email-reg" 
                            type="email" 
                        ...
                        <label for="password-reg" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Contraseña</label>
                        <Password 
                            id="password-reg" 
                            v-model="password" 
                        ...
                        <div class="flex items-center mb-12">
                            <Checkbox v-model="acceptTerms" id="acceptTerms" binary class="mr-2" :disabled="loading"/>
                            <label for="acceptTerms" class="text-surface-900 dark:text-surface-0">
                        ...
                        <div class="text-center">
                            <router-link to="/auth/login" class="font-medium text-primary">
